  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) {
          setItems(Array.isArray(payload.bookmarks) ? payload.bookmarks : [])
        }
      } catch (err) {
        if (active) {
    ...
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-40 border-b border-border bg-background">
        <div className="flex h-12 items-center px-4">
          ...
          <h1 className="ml-2 text-base font-semibold text-foreground">我的收藏</h1>
        </div>
      </header>
      ...
      <div className="space-y-3 px-4 py-4">
        ...
        {items.map((item, index) => {
          ...
          return (
            <Link key={`${tid || "bookmark"}-${index}`} href={href}>
