...
`05_TASK_BOARD.md` still contains old task state, split-era repo assumptions, historical acceptance records, old runtime/storage references, and long audit sections. The file remains useful as history and context, but current code, merged PRs, root README, PR-derived status, and override files are more authoritative.
...
- what command, runtime model, or route owner should be used today.
